...
tol=1e-12;                           % Tolerance for stopping criteria
...
x0=0;   % Initial guess - applies to the fixed point methods, but not bisection.
...
%% Newton method
% Example 3.6.1
df = @(x)2*cos(2*x)+1            % Define the derivative df/dx (Newton needs this!)
[zero,res,niter,itersn]=newton(f,df,x0,tol,nmax)    % Run Newton
% Works well for e.g. x0=0.5, but for e.g. x0=1 or x0=1.04 it takes longer 
% to start converging, because we're near a root of f'. 
% bisection(df,1,1.1,tol,nmax) will give an approximation to this root!
%% Chord method
% Example 3.6.1
...
zeroex=newton(f,df,0,1e-15,100) % Generate an "exact" solution. Actually a highly accurate Newton approximation, since we can't find the root analytically.
